...
if ($action == "doedit") {
	...
	do_update("$db_cmd tests_snmp SET name=\"$test_name\", oid=\"$test_oid\", dev_type=$dev_type $db_end");
} // done editing

if ($action == "dodelete")
{
        do_update("DELETE FROM tests_snmp WHERE id=$test_id");
} // done deleting
...
if (($action == "edit") || ($action == "add"))
{
        ...
        $test_results = do_query("SELECT * FROM tests_snmp WHERE id=$test_id");
        $test_row = mysql_fetch_array($test_results);
